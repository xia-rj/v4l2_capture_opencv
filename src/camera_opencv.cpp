...
//     {
//         struct v4l2_buf#include <opencv2/imgproc.hpp>fer buf;
//         unsigned int i;
...
#include "camera_zhkj.hpp"

int main(int argc, char **argv)
{
    CameraZhkj camera0("/dev/video0"); 
    while (true) 
    {
        // void * yuv422frame;
        
        uint8_t yuv422frame[camera0.SIZE];

        camera0.get_frame((void *)yuv422frame);
        
    //     cv::Mat cvmat(HEIGHT, WIDTH, CV_8UC1);
    //     cv::Mat(HEIGHT, WIDTH, CV_16UC1, (void *)yuv422frame).convertTo(cvmat, CV_8UC1, 1.0 / 256);
        cv::Mat cvmat(camera0.HEIGHT, camera0.WIDTH, CV_8UC1);
        cv::Mat(camera0.HEIGHT, camera0.WIDTH, CV_16UC1, (void *)yuv422frame).convertTo(cvmat, CV_8UC1, 1.0 / 256);
        ...
        for (index; index < hist_size[0]; index++){
            ...
            if (num > cvmat.rows*cvmat.cols/2){
                break;
            }
        }
